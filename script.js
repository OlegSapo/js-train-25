...
async function logRandomNumberAfterSeconds(sec) {
  // Використовуємо try для обробки помилок
  try {
    ...
    const randomNumber = await getRandomNumberAfterSeconds(sec);
...
async function getDataFromUrl(url) {
  ...
  try {
    ...
    res.ok ? console.log("Відповідь вдала") : console.log("Помилка відповіді");
...
async function displayData() {
  ...
  console.log((await res.next()).value);
  console.log((await res.next()).value);
  console.log((await res.next()).value);
}
// Розкоментуйте після виконання завданння
